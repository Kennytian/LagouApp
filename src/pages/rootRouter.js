...
import {getSceneStyle, NavigationStyle, backButtonImage, tabBarStyle} from '../components/styles/routerBase';
...
export default class RootRouter extends Component {
  ...
  render() {
    ...
    return (
      <Router createReducer={reducerCreate} getSceneStyle={getSceneStyle} titleStyle={NavigationStyle.title} titleOpacity={1} backButtonImage={backButtonImage} tabBarStyle={tabBarStyle} navigationBarStyle={NavigationStyle.view} key='modal'>
        <Scene key='root' hideNavBar={false}>
          <Scene key='tabbar' tabs={true}>
            <Scene key='tab1' component={HomeIndex} title='首页' icon={navBarIcon} idx={0} initial={true}/>
            <Scene key='tab2' component={MessageIndex} title='消息' icon={navBarIcon} idx={1} navigationBarStyle={{backgroundColor:'transparent'}}/>
